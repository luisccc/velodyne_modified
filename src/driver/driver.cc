                               ...
                               ros::NodeHandle private_nh)
{
  ...
  private_nh.param("number_echos", config_.number_echos, 1);
  ...
  if ((config_.model == "64E_S2") || 
  ...
  else if (config_.model == "VLS128")
    {
      ...
      {
        packet_rate = 6250*3;             // 6250 determined via try and error
      }
      else
      {
        packet_rate = 6250;             // 6250 determined via try and error
      }
  ...
  time_analysis = node.advertise<std_msgs::Int32>("time_analysis_driver", 10);
}
