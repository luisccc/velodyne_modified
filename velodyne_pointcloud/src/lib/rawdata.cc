...
namespace velodyne_rawdata
{
                              ...
                              double view_width)
  {
    ...
    //ROS_ERROR_STREAM("Max range: " << config_.max_range);
  ...
  int RawData::setup(ros::NodeHandle private_nh)
  {
    ...
    private_nh.param("number_echos", config_.number_echos, 1);   // get number of echos
  ...
  void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase& data)
  {
    ...
    int blocks_actually;
    if (config_.number_echos == 2 && calibration_.num_lasers == 128)
    {
      blocks_actually = 8; //8
    }
    else
    {
      blocks_actually = BLOCKS_PER_PACKET;
    }
    for (int i = 0; i < blocks_actually; i++) {
      ...
      if (raw->blocks[i].header == LOWER_BANK) {
        // lower bank lasers are [32..63] for HDL64
        bank_origin = 32;
      }
      if (calibration_.num_lasers == 128)
      {
        // The modulus allows to later add the number of previously shot lasers to the 
        // variable laser_number
        if (config_.number_echos == 2)
        {
          laser_bank = fmod(floor(i/2),4);             // for dual return
        }
        else
        {
          laser_bank = fmod(i,4); // for single return
        }
      ...
      for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        laser_number = j + 32*laser_bank + bank_origin; // j + 32*laser_bank + bank_origin
